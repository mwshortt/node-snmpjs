...
Receiver.prototype._process_msg = function _process_msg(msg) {
	this._log.debug({
		...
		origin: msg.src,
		snmpmsg: msg
	    }, '[SNMP Service] Ignoring PDU of inappropriate type ' +
...
Receiver.prototype._recv = function _recv(raw, src, conn) {
	...
	try {
		...
		this._log.debug({
			error: err,
			raw: raw,
			origin: src }, '[SNMP Service] Invalid SNMP message');
		return;
	}
	...
	try {
		...
		this._log.warn('[SNMP Service] Error augmenting message', error);
		return;
	}

	this._log.debug({ raw: raw, origin: src, snmpmsg: msg },
	    '[SNMP Service] Received SNMP message');

	try {
		this._process_msg(msg);
	} catch (error) {
		this._log.warn('[SNMP Service] Error processing message', error);
	}
};
