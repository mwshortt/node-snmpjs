	...
	if (typeof (vb) === 'undefined' && req.pdu.op == PDU.GetNextRequest) {
		...
		if (req.pdu.op != PDU.SetRequest && vb != PDU.genErr) {
			this.log.warn({ snmpmsg: req },
			    '[SNMP Service] Provider attempted to set prohibited ' +
...
Agent.prototype._transmit_response = function _transmit_response(sock, rsp) {
	...
	rsp.encode();
	this._log.debug({ raw: rsp.raw, destination: dst, snmpmsg: rsp },
			    '[SNMP Service] Sending SNMP response message');
...
Agent.prototype._process_req = function _process_req(req) {
	...
	if (this._community) {
		if (this._community !== req.community.toString()) {
			this._log.warn('[SNMP Service] Community string provided does not match Agent\'s community string');
			return;
		}
	}
